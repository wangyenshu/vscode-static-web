...
class FileWatcherManager {
    ...
    watchFile(path, callback, pollingInterval, options) {
        ...
        if (this.enabledExperimentalTypeAcquisition && (0, pathMapper_1.looksLikeNodeModules)(path)) {
            watchIds.push(++this.watchId);
            this.watchPort.postMessage({ type: 'watchFile', uri: (0, pathMapper_1.mapUri)(uri, 'vscode-node-modules'), id: watchIds[1] });
        }
        return {
            close: () => {
