    ...
    dispatchEvent(event) {
        switch (event.event) {
            ...
            case protocol_const_1.EventName.createDirectoryWatcher:
                this.createFileSystemWatcher(event.body.id, new vscode.RelativePattern(vscode.Uri.file(event.body.path), event.body.recursive ? '**' : '*'), event.body.ignoreUpdate);
                break;
            case protocol_const_1.EventName.createFileWatcher:
                this.createFileSystemWatcher(event.body.id, new vscode.RelativePattern(vscode.Uri.file(event.body.path), '*'));
                break;
