...
async function startServer(connection, serverConfig) {
    ...
    connection.onRequest(protocol.prepareUpdatePastedLinks, (async (params, token) => {
    ...
    connection.onRequest(protocol.getUpdatePastedLinksEdit, (async (params, token) => {
        const document = documents.get(params.pasteIntoDoc);
        if (!document) {
            return undefined;
        }
        // TODO: Figure out why range types are lying
        const edits = params.edits.map((edit) => lsp.TextEdit.replace(lsp.Range.create(edit.range[0].line, edit.range[0].character, edit.range[1].line, edit.range[1].character), edit.newText));
