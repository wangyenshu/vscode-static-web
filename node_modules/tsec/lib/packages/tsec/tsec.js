 ...
 */
function main(args) {
    if ((0, build_1.isInBuildMode)(args)) {
        return (0, build_1.performBuild)(args.slice(1)) === 0 ? 0 : 1;
    }
    let parsedConfig = ts.parseCommandLine(args);
    if (parsedConfig.errors.length !== 0) {
        // Same as tsc, do not emit colorful diagnostic texts for command line
        // parsing errors.
        ts.sys.write(ts.formatDiagnostics(parsedConfig.errors, report_1.FORMAT_DIAGNOSTIC_HOST));
        return 1;
    }
    ...
    if (parsedConfig.fileNames.length === 0) {
        let tsConfigFilePath = parsedConfig.options.project;
        ...
        const parseConfigFileHost = Object.assign(Object.assign({}, ts.sys), { onUnRecoverableConfigFileDiagnostic: (diagnostic) => {
                ts.sys.write(ts.formatDiagnostic(diagnostic, report_1.FORMAT_DIAGNOSTIC_HOST));
                ts.sys.exit(1);
            } });
        parsedConfig = ts.getParsedCommandLineOfConfigFile(tsConfigFilePath, parsedConfig.options, parseConfigFileHost);
    }
    const diagnostics = [...parsedConfig.errors];
    const compilerHost = (0, compiler_host_1.createCompilerHost)(parsedConfig);
    const program = ts.createProgram(parsedConfig.fileNames, parsedConfig.options, compilerHost);
    diagnostics.push(...ts.getPreEmitDiagnostics(program), ...(0, build_1.performCheck)(program, compilerHost));
    ...
    const reportDiagnostics = (0, report_1.createDiagnosticsReporter)(parsedConfig.options);
    const errorCount = reportDiagnostics(ts.sortAndDeduplicateDiagnostics(diagnostics), /*withSummary*/ true);
    return errorCount === 0 ? 0 : 1;
}
process.exitCode = main(process.argv.slice(2));
//# sourceMappingURL=tsec.js.map